...
class Function(object):
    ...
    def _get_name(self, names, autonames):
        if self not in names and self.ntop > 0:
            ...
            autonames[self.params.name] += 1
            names[self] = self.type_name + str(autonames[self.params.name])
        return names[self]

    def _get_top_name(self, top, names, autonames):
        if top not in names:
            autonames[self.name] += 1
            if self.ex_top != None:
                assert(self.ex_top.__len__ >= autonames[self.name]), 'ex_top.len:{}, autonames:{}'.format(self.ex_top.__len__, autonames[self.name])
                #print '{}---{}'.format(autonames[self.name] - 1, self.ex_top)
                names[top] = self.ex_top[ autonames[self.name] - 1]
            else:
                names[top] = top.fn.type_name + str(autonames[self.name])
        return names[top]
