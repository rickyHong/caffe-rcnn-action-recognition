...
def parse_args():
    ...
    parser.add_argument('--diff', dest='diff_file',
                        help='difficult file',
                        default=None, type=str)
...
def prepare_data(args):
    ...
    if not os.path.isfile(args.diff_file):
        print 'Difficult File({}) does not exist'.format(args.diff_file)
        sys.exit(1)
    gt_file = open(args.gt_file)
    ans_file = open(args.ans_file)
    diff_file = open(args.diff_file)
    # Handle
    ...
    boxes_num_res = 0
    while True:
        gt_line, ans_line, diff_line = gt_file.readline(), ans_file.readline(), diff_file.readline()
        if gt_line == '':
            assert (gt_line == ans_line), 'GT File Done But Results File unfinished, {}'.format(ans_line)
            assert (gt_line == diff_line), 'GT File Done But Diff File unfinished, {}'.format(diff_line)
            break
        gt_line, ans_line, diff_line = gt_line.strip('\n').split(' '), ans_line.strip('\n').split(' '), diff_line.strip('\n').split(' ')
        ...
        assert (len(diff_line) == 2 and diff_line[0] == '#'), 'First Line of Diff File Must Be \'# ID\''
        assert (ans_line[1] == gt_line[1]), 'ID og GT and Result Must Be Same'
        gt_line, ans_line, diff_line = gt_file.readline().strip('\n'), ans_file.readline().strip('\n'), diff_file.readline().strip('\n')
        assert (gt_line == ans_line), 'Image Does not Euqal : {} and {}'.format(gt_line, ans_line)
        assert (gt_line == diff_line), 'Image Does not Euqal : {} and {}'.format(gt_line, diff_line)
        images.append(gt_line)

        gt_line, ans_line, diff_line = gt_file.readline().strip('\n'), ans_file.readline().strip('\n'), diff_file.readline().strip('\n')
        num_gt, num_ans, num_diff = int(gt_line), int(ans_line), int(diff_line)
        assert num_gt == num_diff
        ...
        for index in range(num_gt):
            gt_line, diff_line = gt_file.readline().strip('\n').split(' '), diff_file.readline().strip('\n').split(' ')
            for jj in range(gt_line.count('')):
                gt_line.remove('')
            for jj in range(diff_line.count('')):
                diff_line.remove('')
            assert (len(gt_line) == 5), 'Ground Truth : label x1 y1 x2 y2 , not {}'.format(gt_line)
            ...
            obj_struct['difficult'] = int(diff_line[0])
