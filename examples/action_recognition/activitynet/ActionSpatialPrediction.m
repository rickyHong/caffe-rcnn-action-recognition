function prediction = VideoSpatialPrediction(rgb_path, mean_file, net)
num=50;
%vidObj = VideoReader(vid_name);
%duration = vidObj.NumberOfFrames;
imglist = dir([rgb_path,'/*image*.jpg']);
...
for i = 1:num
    img = single(imresize(imread(sprintf('%s/image_%08d.jpg',rgb_path,(i-1)*step)),[256,340]));
    rgb(:,:,:,i) = img;
